...
class ProductController extends Controller
{
    public function create(Request $request,$userId,$catId){
        ...
        if($val->fails()){
            ...
            $prod->material=$request['material'];
            $prod->brand_name=$request['brand_name'];
            $prod->quantity=$request['quantity'];
            $prod->color=$request['color'];
            $prod->weight=$request['weight'];
            $prod->size=$request['size'];
            if(isset($prod->reviews_number)){        
                $prod->reviews_number=$request['reviews_number'];
            }
            if(isset($prod->star_number)){        
                $prod->star_number=$request['star_number'];
            }
            if(isset($prod->origin)){        
                $prod->origin=$request['origin'];
            }
            if(isset($prod->sold_number)){        
                $prod->sold_number=$request['sold_number'];
            }
